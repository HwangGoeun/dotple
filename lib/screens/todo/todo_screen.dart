              ...
              children: [
                ...
                Container(
                  ...
                  child: TextButton(
                    onPressed: () {
                      // 날짜 비교
                      if (_todoFirstDay.isAfter(_todoLastDay)) {
                        // 에러 메시지 표시
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('시작 날짜가 종료 날짜보다 늦을 수 없습니다.'),
                            duration: Duration(seconds: 2),
                          ),
                        );
                        return; // 날짜 비교 실패 시 반환
                      }
                      Todo newTodo = Todo(
                        name: _todoName,
                      ...
                      DateTime date = _todoFirstDay;
                      while (date.isBefore(_todoLastDay) ||
                          date.isAtSameMomentAs(_todoLastDay)) {
                        eventProvider.addToEventsList(
                            date,
                            Todo(
                              name: _todoName,
                              startDay: _todoFirstDay,
                        ...
                        date = date.add(const Duration(days: 1)); // 다음 날짜로 이동
                      }
